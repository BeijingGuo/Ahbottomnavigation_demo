...
public class AHHelper {
	...
	public static void updateDrawableColor(final Drawable nodrawable,final Drawable sele_drawable,final int no_color,
										   final ImageView imageView,final @ColorInt int fromColor,
										   @ColorInt int toColor) {
		ValueAnimator colorAnimation = ValueAnimator.ofObject(new ArgbEvaluator(), fromColor, toColor);
		colorAnimation.setDuration(150);
		colorAnimation.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
			...
			public void onAnimationUpdate(ValueAnimator animator) {
				...
				if(fromColor==no_color)
				{
					imageView.setImageDrawable(nodrawable);
				}
				else
				{
					imageView.setImageDrawable(sele_drawable);
				}
			}
		});
		colorAnimation.start();
